import sys
# to access command line arguments and used for exiting the running code

from Maze import *
from show import *
from Algorithm import *
from AlgorithmEvaluator import*
# Import custom classes and functions from other files (Maze.py, show.py, and Algorithm.py)
...
size = width, height = 600, 600: Defines the screen size as a 600x600 window.
pygame.display.set_mode(size): Creates a window of the specified size.
'''

RectButtonFont = pygame.font.Font("OpenSans-Regular.ttf", 16)
CirButtonFont = pygame.font.Font("OpenSans-Regular.ttf", 40)
'''
Pygame is a graphics library, which means everything you see on the screen is drawn—
whether it's images, shapes, or text.
pygame.font.Font("OpenSans-Regular.ttf", 16): Loads the "OpenSans-Regular.ttf" font with size 16 for rectangular buttons.
pygame.font.Font("OpenSans-Regular.ttf", 40): Loads the same font but with size 40 for circular buttons.
'''
...
TRAIN = 50            #Number of training iterations for Q-learning.
PADDING = 32      #Defines spacing for UI elements.
RADIUS = 40         #Radius for circular buttons.
board_height = height-4*PADDING
board_width = width-4*PADDING # board size
v_cells = 30
h_cells = 30 # maze width
...
eval_button = CirButton(
    center=(17*PADDING+10, 15*PADDING), radius=RADIUS, text="E",
    textcolor=colors["black"], circolor=colors["white"],
    screen=screen, font=CirButtonFont)
...
while True:
    ...
    if not SEARCH:
        ...
        if left == 1:
            ...
            elif eval_button.distance(mouse) < RADIUS:
                ALGO = "Evaluate"
                eval_button.color_change(colors["yellow"])
    ...
    # search start
        ...
        if ALGO == "Dijkstra":
            ...
            evaluator = AlgorithmEvaluator(board, num_mazes=1, train_q=1)
        ...
        if algorithm.find:
            algorithm.output()
            path_len = len(board.path)
            end_time = time.time()
            print(f"\n Path length: {path_len} | Time: {round(end_time - start_time, 3)} seconds")
        else:
